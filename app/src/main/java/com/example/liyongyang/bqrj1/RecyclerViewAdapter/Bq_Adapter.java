...
public class Bq_Adapter extends RecyclerView.Adapter<Bq_Adapter.LinearViewHolder> {
    private Context mContext;
    private int mWidth;//获取的宽度
    private Typeface mTypeface;

    public Bq_Adapter(Context context, Typeface typeface, int width){//构造函数从外部获取需要的量
    ...
    @NonNull
    @Override
    public Bq_Adapter.LinearViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        return new LinearViewHolder(LayoutInflater.from(mContext).inflate(R.layout.rvitem, viewGroup, false));//f返回一个对象，这个对象就是能实现对item操作的一个对象
    }

    @Override
    public void onBindViewHolder(@NonNull Bq_Adapter.LinearViewHolder viewHolder, int i) {
